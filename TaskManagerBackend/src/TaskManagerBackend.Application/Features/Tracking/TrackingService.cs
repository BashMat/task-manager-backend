                             ...
                             IDateTimeService dateTimeService) : ITrackingService
{
                                                                                         ...
                                                                                         CancellationToken cancellationToken)
    {
        ...
        if (request.TrackingLogEntryStatusId.HasValue &&
            request.TrackingLogEntryStatusId.Value != logEntry.TrackingLogEntryStatusId)
        {
            TrackingLogEntryEntity? topmostTrackingLogEntryEntity =
                await trackingRepository.GetTopOrderedTrackingLogEntryByStatusId(request.TrackingLogEntryStatusId.Value,
                                                                                 cancellationToken);
            
            logEntry.ChangeStatusToByUser(request.TrackingLogEntryStatusId.Value,
                                          userId,
                                          topmostTrackingLogEntryEntity,
                                          dateTimeService.UtcNow);
        }
