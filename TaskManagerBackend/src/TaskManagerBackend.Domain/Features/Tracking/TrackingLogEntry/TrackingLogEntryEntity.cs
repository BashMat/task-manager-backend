...
public class TrackingLogEntryEntity : IAuditedEntity
{
    ...
    public void ChangeStatusToByUser(int value,
                                     int userId,
                                     TrackingLogEntryEntity? targetStatusTopmostTrackingLogEntry,
                                     DateTime dateTime)
    {
        if (TrackingLogEntryStatusId != value)
        {
            TrackingLogEntryStatusId = value;
            OrderIndex = OrderIndexCalculator.CalculateTargetOrderIndex(targetStatusTopmostTrackingLogEntry?.OrderIndex);
            UpdatedBy = userId;
            UpdatedAt = dateTime;
        }
    }
