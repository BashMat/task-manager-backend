...
public class WhenEditingTrackingLogEntry(MsSqlTests fixture) : TrackingTestBase(fixture)
{
    ...
    [Fact]
    public async Task EditingTrackingLogEntrySetsOrderIndexToZeroIfThereAreNoEntries()
    {
        TrackingLogGetResponse createdTrackingLog = await CreateTrackingLogAndValidateResponse();
        TrackingLogEntryStatusGetResponse createdSourceTrackingLogEntryStatus =
            await CreateTrackingLogEntryStatusAndValidateResponse(createdTrackingLog.Id);
        ...
        TrackingLogEntryGetResponse createdTrackingLogEntry =
            await CreateTrackingLogEntryAndValidateResponse(createdTrackingLog.Id,
                                                            createdSourceTrackingLogEntryStatus.Id,
                                                            orderIndex: Faker.Random.Double(0.1, Constants.MaxOrderIndex + 0.1));

        HttpResponseMessage response = await EditTrackingLogEntry(createdTrackingLogEntry.Id,
                                                                  new Optional<string>(),
                                                                  new Optional<int>(createdTargetTrackingLogEntryStatus.Id),
                                                                  new Optional<string>());
        ServiceResponse<TrackingLogEntryGetResponse>? content = 
            await response.Content.ReadFromJsonAsync<ServiceResponse<TrackingLogEntryGetResponse>>();
        ...
        content.Data.Should().NotBeNull();
        content.Data.Id.Should().Be(createdTrackingLogEntry.Id);
        content.Data.Status.Should().BeEquivalentTo(createdTargetTrackingLogEntryStatus);
        content.Data.OrderIndex.Should().Be(0m);
        content.Data.OrderIndex.Should().NotBe(createdTrackingLogEntry.OrderIndex);
        content.Success.Should().BeTrue();
        content.Message.Should().BeNull();
    }
    
    [Fact]
    public async Task EditingTrackingLogEntrySetsOrderIndexToZeroIfThereAreEntriesWithZeroOrderIndex()
    {
        TrackingLogGetResponse createdTrackingLog = await CreateTrackingLogAndValidateResponse();
        TrackingLogEntryStatusGetResponse createdSourceTrackingLogEntryStatus =
            await CreateTrackingLogEntryStatusAndValidateResponse(createdTrackingLog.Id);
        ...
        TrackingLogEntryGetResponse createdTrackingLogEntry =
            await CreateTrackingLogEntryAndValidateResponse(createdTrackingLog.Id,
                                                            createdSourceTrackingLogEntryStatus.Id,
                                                            orderIndex: Faker.Random.Double(0.1, Constants.MaxOrderIndex + 0.1));
        TrackingLogEntryGetResponse trackingLogEntryAtTargetStatus =
            await CreateTrackingLogEntryAndValidateResponse(createdTrackingLog.Id,
                                                            createdTargetTrackingLogEntryStatus.Id,
                                                            orderIndex: 0);
        ...
        content.Data.Status.Should().BeEquivalentTo(createdTargetTrackingLogEntryStatus);
        content.Data.OrderIndex.Should().Be(0m);
        content.Data.OrderIndex.Should().Be(trackingLogEntryAtTargetStatus.OrderIndex);
        content.Data.OrderIndex.Should().NotBe(createdTrackingLogEntry.OrderIndex);
        content.Success.Should().BeTrue();
        content.Message.Should().BeNull();
    }
    ...
    [Fact]
    public async Task EditingTrackingLogEntrySetsOrderIndexToTopIfTopEntryHasNonZeroOrderIndex()
    {
        TrackingLogGetResponse createdTrackingLog = await CreateTrackingLogAndValidateResponse();
        TrackingLogEntryStatusGetResponse createdSourceTrackingLogEntryStatus =
            await CreateTrackingLogEntryStatusAndValidateResponse(createdTrackingLog.Id);
        ...
        TrackingLogEntryGetResponse createdTrackingLogEntry =
            await CreateTrackingLogEntryAndValidateResponse(createdTrackingLog.Id,
                                                            createdSourceTrackingLogEntryStatus.Id,
                                                            orderIndex: Faker.Random.Double(100, Constants.MaxOrderIndex + 0.1));
        await CreateTrackingLogEntryAndValidateResponse(createdTrackingLog.Id,
                                                        createdTargetTrackingLogEntryStatus.Id,
                                                        orderIndex: 1);
        ...
        content.Data.Id.Should().Be(createdTrackingLogEntry.Id);
        content.Data.Status.Should().BeEquivalentTo(createdTargetTrackingLogEntryStatus);
        content.Data.OrderIndex.Should().Be(0.5m);
        content.Success.Should().BeTrue();
        content.Message.Should().BeNull();
    }
